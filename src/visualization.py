...
def plot_conf_mat(y_true, y_pred, class_names, normalize=True, title=None, cmap=plt.cm.viridis, text=True, width=8, height=8 ):
  """
...
def plot_tpr_fpr_vs_thresholds(y_train, anomaly_scores, ax, threshold_selected=None):
  """
  ...
  ax.legend(loc='upper center')
  ax.grid(True)
  #ax.set_xticks(np.arange(min(thresholds),max(thresholds),(max(thresholds) -min(thresholds) )/10  ))
  #ax.xaxis.set_major_formatter(FormatStrFormatter('%.2f'))
  ax.set_yticks(np.arange(0, 1+0.09, 0.1))
  
  if threshold_selected != None:
    ax.axvline(threshold_selected, 0, 1, color='r', linestyle='-.', label="Threshold selected")
...
def evaluate_anomaly_detector(y_train, anomaly_scores, threshold_selected=None):
  ...
  plot_precision_recall_vs_thresholds(y_train, anomaly_scores, axs[1,0], threshold_selected)
  plot_tpr_fpr_vs_thresholds(y_train, anomaly_scores, axs[1,1], threshold_selected)
